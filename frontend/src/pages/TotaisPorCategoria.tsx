...
import { getTotaisPorCategoria, type RelatorioTotaisCategoria } from '../api/relatorios.ts'

/**
 * Pagina de Consulta de Totais por Categoria (OPCIONAL)
 * 
 * Funcionalidade:
 ...
 *   * Resumo geral com totalizacoes de todas as categorias
 */
export default function TotaisPorCategoria() {
  const [relatorio, setRelatorio] = useState<RelatorioTotaisCategoria | null>(null)
      ...
      {!loading && relatorio && (
        <>
          ...
          <div className="list-container">
            <h3>Detalhamento por Categoria ({relatorio.categorias.length})</h3>
                ...
                <tbody>
                  {relatorio.categorias.map((categoria) => (
                    <tr key={categoria.categoriaId}>
                      ...
                      <td className="text-green">{formatarMoeda(categoria.totalReceitas)}</td>
                      <td className="text-red">{formatarMoeda(categoria.totalDespesas)}</td>
                      <td className={getClasseSaldo(categoria.saldo)}>
                        <strong>{formatarMoeda(categoria.saldo)}</strong>
                      </td>
                    </tr>
                  ))}
                </tbody>
